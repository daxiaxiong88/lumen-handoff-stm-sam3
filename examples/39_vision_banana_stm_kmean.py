...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parent / "vision_banana_openclass_outputs"
...
def run(seg, tag, path, output_dir):
    im = to_work(np.asarray(Image.open(path).convert("RGB")))
    Image.fromarray(im).save(output_dir / f"{tag}_input.png")
    print(f"\n=== {tag}  {im.shape} ===", flush=True)
    gen, masks = open_class(seg, im)
    Image.fromarray(gen).save(output_dir / f"{tag}_openclass_raw.png")
    ov = overlay_each(im, gen, masks)
    Image.fromarray(ov).save(output_dir / f"{tag}_openclass_overlay.png")
    ...
    cells = [im, gen, ov]
    titles = ["input", "model RGB", f"{len(masks)} features (coloured)"]
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f"{tag}_openclass_panel.png", dpi=110, bbox_inches="tight")
...
def main():
    ...
    for path in args.images:
        if path.exists():
            run(seg, path.stem, path, args.output_dir)
        else:
